...
new const eventSlots[21] = {-1, -1, 16, -1, 11, -1, 28, 42, 18, 31, 32, 32, 58, 15, 15, 15, 15, 15, 10, 33, 30}; /* -1 = unlimited slots */
...
new Float:pursuitVehicles[][] = {
	{565.6188,-1244.3657,16.9157,295.8679}, // 0 etc
	...
	{612.4984,-1224.6945,17.8323,289.7163}, // Pursuit vehicle  10
	{550.2395,-1282.4430,17.4245,335.9597}, // Maverick 1   11
...
public event_OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(Event_ID != -1)
	{
	    if(GetPVarInt(playerid, "InEvent") == 1)
		{
			...
			if((Event_ID == JEFFTDM || Event_ID == AREA51 || Event_ID == ARMYVSTERRORISTS || Event_ID == NAVYVSTERRORISTS || Event_ID == COMPOUND || Event_ID == OILRIG || Event_ID == DRUGRUN || Event_ID == PURSUIT || Event_ID == PLANE) && killerid != INVALID_PLAYER_ID)
			{
				...
				if(GetPVarInt(playerid, "Team") == GetPVarInt(killerid, "Team"))
				{
					new string[128];
					GivePlayerMoney(playerid, 100);
					GivePlayerMoney(killerid, -1000);
	...
	if(Event_Delay == 0)
	{
		...
		else
		{
			...
			switch(Event_ID)
			{
				...
				case PLANE: plane_OneSecond();
				case CONSTRUCTION: construction_OneSecond();
			}
		}
	}
...
timer SumoFallCheck[1000]()
{
	foreach(Player, i)
	{
		if(GetPVarInt(i, "InEvent") == 1)
		{
			...
			if(vz < 25.0 || GetPlayerState(i) != PLAYER_STATE_DRIVER)
			{
				PlayerLeftEvent(i);
			}
		}
...
public EventStart(type, playerid)
{
	...
	*/
	switch(type)
	{
		...
		case PLANE: plane_EventStart(playerid);
		case CONSTRUCTION: construction_EventStart(playerid);
 	}
...
public EndEvent()
{
	...
	foreach(new i : Player)
	{
		...
		{
		    if(Event_ID == JEFFTDM || Event_ID == ARMYVSTERRORISTS || Event_ID == DRUGRUN || Event_ID == PURSUIT || Event_ID == AREA51 || Event_ID == NAVYVSTERRORISTS || Event_ID == OILRIG || Event_ID == COMPOUND || Event_ID == PLANE || CONSTRUCTION)
		    {
		        SetPVarInt(i, "MotelTeamIssued", 0);
	...
	if(Event_ID == DRUGRUN || Event_ID == PURSUIT || Event_ID == NAVYVSTERRORISTS || Event_ID == COMPOUND || Event_ID == ARMYVSTERRORISTS || Event_ID == PLANE)
	{
...
{
    if(Team1_Motel == 3 && Team2_Motel == 27)
	{
		return SendClientMessage(playerid, COLOR_NOTICE, "                This event is full");
	}
    SetPlayerArmour(playerid, 99);
	...
	if(Team1_Motel == 0 || Team2_Motel == 10 || Team2_Motel == 20)
	{
		Team1_Motel++;
		SetPVarInt(playerid, "MotelTeamIssued", 1);
		SetPlayerColor(playerid, COLOR_BLUE);
...
public plane_PlayerLeftEvent(playerid)
{
	...
	{
	    Motel_Team--;
		format(msg, sizeof(msg), "[EVENT SCORE]: Pilots %d - %d Hobos", Motel_Team, Team1_Motel);
		SendClientMessageToAll(COLOR_NOTICE, msg);
	}
...
public pursuit_EventStart(playerid)
{
	...
	for(new i = 0; i < 25; i++)
	{
		if(i < MAX_EVENT_VEHICLES)
		{
			if(i == 10)     // If you change this, also change the ones on top!!
			{
			...
			else if (i == 11 || i == 24)    // If this is changed, change accordingly on top
 			{
...
CMD:event(playerid, params[])
{
	if(IsAdmin(playerid, 1))
	{
		...
		{
		    format(string, sizeof(string), "[USAGE]: {%06x}/event {%06x}[Start/End/Setbrawlpoint/Add]", COLOR_WHITE >>> 8, COLOR_SYNTAX >>> 8);
