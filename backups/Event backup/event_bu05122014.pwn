...
new const eventSlots[MAX_EVENTS] = {-1, -1, 16, -1, 11, -1, 28, 42, 18, 31, 32, 32, 58, 15, 15, 15, 15, 15, 26, 33, 30, 26}; /* -1 = unlimited slots */
...
public EndEvent()
{
	...
	foreach(new i : Player)
	{
		...
		{
		    if(Event_ID == JEFFTDM || Event_ID == ARMYVSTERRORISTS || Event_ID == DRUGRUN || Event_ID == PURSUIT || Event_ID == HIGHSPEEDPURSUIT || Event_ID == AREA51 || Event_ID == NAVYVSTERRORISTS || Event_ID == OILRIG || Event_ID == COMPOUND || Event_ID == PLANE || CONSTRUCTION)
		    {
		        SetPVarInt(i, "MotelTeamIssued", 0);
