...
new Float:sumoSpawnsType5[15][4] = {
	{4131.7363,1043.4788,500.5833,0.0959},
	{4118.9180,1073.8458,500.5863,179.4576},
	{4201.2710,1094.3248,500.5785,180.5106},
	{4209.3691,913.4738,524.2874,185.5211},
	{3797.7000,889.3711,518.8420,90.4424},
	{3949.2146,1033.0588,500.7086,168.6903},
	{3954.4568,1193.3824,500.6860,356.4665},
	{3929.8860,1294.8730,500.5077,90.2096},
	{4038.4854,1295.3348,500.4798,269.5747},
	{4277.8950,1256.9875,504.7538,219.7306},
	{4052.6091,1000.3039,500.5995,270.0812},
	{4156.8994,945.4161,500.5934,264.0971},
	{3978.2166,1257.1235,500.6043,270.0741},
	{4000.4585,1334.8267,500.5608,89.8899},
	{4129.8804,1313.4404,500.5281,125.3335}
};
...
public monster_EventStart(playerid)
{
	...
	format(string, sizeof(string), "[EVENT]: %s %s has started the {%06x}Monster Sumo event. {%06x}30 seconds before it starts, type /join! Map by {%06x}Rayoo.", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_CMDNOTICE >>> 8,COLOR_WARNING >>> 8);
...
public banger_EventStart(playerid)
{
	...
	Event_ID = BANGERSUMO;
	format(string, sizeof(string), "[EVENT]: %s %s has started the {%06x}Banger Sumo event. {%06x}30 seconds before it starts, type /join! Map by {%06x}Tsar.", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_CMDNOTICE >>> 8, COLOR_WARNING >>> 8);
	SendClientMessageToAll(COLOR_CMDNOTICE, string);
	IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
	Event_InProgress = 0;
	Event_Delay = 30;
...
public sandking_EventStart(playerid)
{
	...
	Event_ID = SANDKSUMO;
	format(string, sizeof(string), "[EVENT]: %s %s has started the {%06x}SandKing Sumo event. {%06x}30 seconds before it starts, type /join! Map by {%06x}RakGuy.", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_CMDNOTICE >>> 8, COLOR_WARNING >>> 8);
	SendClientMessageToAll(COLOR_CMDNOTICE, string);
	IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
	Event_InProgress = 0;
	Event_Delay = 30;
...
public sandkingR_EventStart(playerid)
{
	...
	format(string, sizeof(string), "[EVENT]: %s %s has started the {%06x}SandKing Sumo Reloaded event. {%06x}30 seconds before it starts, type /join! Map by {%06x}RakGuy.", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_CMDNOTICE >>> 8, COLOR_WARNING >>> 8);
	SendClientMessageToAll(COLOR_CMDNOTICE, string);
	IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
	Event_InProgress = 0;
	Event_Delay = 30;
...
public derby_EventStart(playerid)
{
	...
	format(string, sizeof(string), "[EVENT]: %s %s has started the {%06x}Destruction Derby event. {%06x}30 seconds before it starts, type /join! Map by {%06x}Hiro.", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_CMDNOTICE >>> 8, COLOR_WARNING >>> 8);
	SendClientMessageToAll(COLOR_CMDNOTICE, string);
	IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
	Event_InProgress = 0;
	Event_Delay = 30;
