...
{
	new spd_msg[128];
	format(spd_msg, sizeof(spd_msg), "%s(%i) has been shown DialogID: %i.", PlayerName(playerid), playerid, dialogid);
	DialogLog(spd_msg);
	if(!strlen(caption))
	    strins(caption, " ", 0, 10);
	if(!strlen(info))
...
public AntiCheat_GivePlayerMoney(playerid, amount)
{
	...
	new mon_msg[128];
	format(mon_msg, sizeof(mon_msg), "%s's money has been scriptly given $%i", PlayerName(playerid), amount);
	SetMoneyLog(mon_msg);
...
public AntiCheat_ResetPlayerMoney(playerid)
{
	...
	new mon_msg[128];
	format(mon_msg, sizeof(mon_msg), "%s's money has been scriptly reset.", PlayerName(playerid));
	SetMoneyLog(mon_msg);
...
public AntiCheat_SetPlayerMoney(playerid, amount)
{
	...
	new mon_msg[128];
	format(mon_msg, sizeof(mon_msg), "%s's money has been scriptly set to $%i", PlayerName(playerid), amount);
	SetMoneyLog(mon_msg);
	return GivePlayerMoney(playerid, amount);
}
...
/*hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	printf("ODR_AC");
	new string[512];
	format(string, sizeof(string), "%s(%i) got Dialog Response for DialogID: %i", PlayerName(playerid), playerid, dialogid);
	DialogLog(string);
	...
	{
	    format(string, sizeof(string), "%s(%i) is possibly using FakeDialog.cs(%i).", PlayerName(playerid), playerid, dialogid);
        DialogLog(string);
	}
...
hook OnPlayerUpdate(playerid)
{
	...
	if(PlayerLastUpdated[playerid][acs_vehicle]  < time)
	{
		...
		{
			if(PlayerData[playerid][ac_lastvehicle] != INVALID_VEHICLE_ID && IsValidVehicle(PlayerData[playerid][ac_lastvehicle]))
		    {
				new
					Float:hp;
