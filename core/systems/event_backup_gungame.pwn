...
public event_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_EVENTS)
	{
	    ...
	    switch(listitem)
	    {
			...
			case GUNGAME:
			{
				SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: Gun Game is currently disabled due to performance issues.");
			}
			case CEM:
			{
				if(Event_InProgress != -1)
...
public PlayerJoinEvent(playerid)
{
	...
	if(Event_ID != MADDOGG && Event_ID != BIGSMOKE && Event_ID != BRAWL)
 	{
