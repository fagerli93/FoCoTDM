...
forward FoCo_Event_Rejoin_Func(Rejoin);
...
new CEM_Selected_list_dialog_show[MAX_PLAYERS];
	...
	*************************************************/
forward CEM_OnPlayerDeath(playerid, killerid, reason);
enum
{
	NILVALUE,
	TDM,
...
{
	if(Get_Event_InProgress()!=-1)
 	{
		if(Event_Currently_On()==EVENT_CEM)
		{
			if(GetPVarInt(playerid, "InEvent")==1)
			{
		   		if(CEM_RUNNING==FFA_SUMO || (CEM_RUNNING==RACE))
				{
	...
	if(Get_Event_InProgress()!=-1)
	{
		if(checkpointid==CEM_Race_CPIDs[CEM_Race_CurrCP[playerid]])
		{
		    if(CEM_Race_CurrCP[playerid]==CEM_Race_CPCount-1)
		    {
				...
				{
					format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %s won the custom race event.", PlayerName(playerid));
					SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
				}
				else
				{
					format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %i - %s.",CEM_FinishedCount, PlayerName(playerid));
					SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
				}
...
public CEM_OnPlayerDeath(playerid, killerid, reason)
{
    //DebugMsg("CEM_OPD Called");
	if(GetPVarInt(playerid, "InEvent")==1)
	{
		if(Get_Event_InProgress()!=-1)
		{
...
{
	if(Get_FoCo_Event_Died(playerid)==1)
	{
	    if(CEM_RUNNING==TDM)
	    {
...
{
	Edit_FoCo_Event_Died(playerid,0);
	return 1;
}

hook OnGameModeInit()
{
	Edit_Event_InProgress(-1);
	CEM_TDM_LSTSpawner=-1;
}
/*******************************************************************************
  			...
  			else
  			{
			    ...
			    switch(listitem)
			    {
			        case 0:
			        {
						...
						if(CEM_Spawns_Set!=1)
						{
						    SendClientMessage(playerid, COLOR_SYNTAX, "[USAGE]: Use /settdmspawna && /settdmspawnb");
						}
						else
						{
						    SendClientMessage(playerid, COLOR_NOTICE, "[NOTICE]: Another admin already set two Coordinates, use it or replace it using /settdmspawna && /settdmspawnb.");
						}
	    				return 1;
					}
			        case 1:
  			...
  			if(!response)
		    {
  			...
  			else
  			{
			    ...
			    switch(listitem)
			    {
			        ...
			        {
			            CEM_Event_Type[playerid]=0;
			            SendClientMessage(playerid, COLOR_NOTICE, "[CEM]: Use /setracespawn to create spawn and /resetracespawn to reset.");
						return 1;
					}
			        case 1:
			        {
			        ...
			        {
			            CEM_Event_Type[playerid]=0;
			            SendClientMessage(playerid, COLOR_NOTICE, "[CEM]: Use /setracecp to create spawn and /resetracecp to reset.");
						return 1;
					}
					case 3:
			        {
					...
					case 4:
					{
						...
						{
						    SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: Already one event is in progress");
						}
					}
			    }
			}
			    ...
			    switch(CEM_Selected_list[playerid])
			    {
					case 3:
					{
					    ...
					    else
					    {
						    ...
						    if(CEM_Race_VehID > 609 || CEM_Race_VehID < 400)
						        CEM_Race_VehID = 411;
						    //VehicleID Choose
						    CEM_GetDetails(playerid, 1,1, "");
						}
  			...
  			if(!response)
		    {
  			...
  			else
  			{
			    ...
			    switch(listitem)
			    {
			        ...
			        {
			            CEM_Event_Type[playerid]=0;
			            SendClientMessage(playerid, COLOR_NOTICE, "[CEM]: Use /setffaspawn to create spawn and /resetffaspawn to reset.:");
						return 1;
					}
					case 2:
			        {
					...
					case 3:
					{
 						if(Get_Event_InProgress()==-1)
					    {
							...
							{
							    SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: Select vehicle.");
							}
						}
						else
						{
						    	SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: An event is already in progress.");
						}
					}
			    }
			}
    		    ...
    		    switch(CEM_Selected_list[playerid])
			    {
					case 2:
					{
					    ...
					    else
					    {
						    ...
						    if(CEM_FFA_VehID > 609 || CEM_FFA_VehID < 400)
						        CEM_FFA_VehID = 444;
						    //VehicleID Choose
...
///////////////////////////////FUNCTION DETOUR//////////////////////////////////
*******************************************************************************/
new Timer:CEM_TimerID;
new TimerTime;
timer CEM_EventStart[1000]()
{
	if(Get_Event_InProgress()==0)
	{
	    if(TimerTime % 3 == 0)
	    {
	 		foreach(Player,i)
	...
	if(TimerTime==0)
	{
	    ...
	    if(CEM_JoinCount>1)
	    {
			...
			SendClientMessageToAll(COLOR_NOTICE, "[EVENT]: The custom event has started!");
		    Edit_Event_InProgress(1);
		}
		else if(CEM_JoinCount<=1)
		{
		    SendClientMessageToAll(COLOR_NOTICE, "[EVENT]: The event ended due to not enough players participated.");
...
stock CEM_TIMER()
{
    CEM_TimerID = repeat CEM_EventStart();
	return 1;
}
...
stock CEM_CallFunction(FuncID, playerid)
{
	...
	if(FuncID==1)
	{
	    ...
	    if(Get_Event_InProgress() == 0 && CEM_RUNNING != FFA_DM)
		{
		    CEM_JoinCount++;
		    format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: Event will start in %i seconds.", TimerTime);
		    SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
			TogglePlayerControllable(playerid, 0);
		}
	}
	//DebugMsg("Calling FuncIDs");
	/*************************************************
	                      TDM
	*************************************************/
	if(CEM_RUNNING==TDM)
	...
	*************************************************/
	...
	{
        //DebugMsg("Resetting InEvent & PlayerStatus @FuncID");
...
stock CEM_TDM_EventStart(playerid)
{
	foreach(Player, i)
    {
	    Edit_FoCo_Event_Died(i, 0);
	    SetPVarInt(i, "InEvent", 0);
    }
    format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %s %s has started a {%06x}Custom TDM {%06x}event.  Type /join!", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_NOTICE >>> 8);
    SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
    Edit_Event_InProgress(0);
	Edit_Event_ID(EVENT_CEM);
	CEM_RUNNING=TDM;
...
stock CreateCircleAroundpoint(Float:x, Float:y, &Float:x2, &Float:y2, Float:radius)
{
	x2 = x + floatcos(3.6,degrees) * radius;
	y2 = y + floatsin(3.6,degrees) * radius;
	return 1;
}
...
stock CEM_TDM_Join(playerid)
{
	...
	SetPlayerVirtualWorld(playerid, CEM_TDM_WORLD);
	...
	SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_INEVENT);
	new y_count = 0, temp;
	new Float:Circ_Pos[2];
	if(CEM_TDM_JoinCount%2==0)
	{
		...
		CreateCircleAroundpoint(CEM_TDM_Spawns[0][0], CEM_TDM_Spawns[0][1], Circ_Pos[0], Circ_Pos[1], float(random(10)+1));
  		SetPlayerPos(playerid, Circ_Pos[0], Circ_Pos[1], CEM_TDM_Spawns[0][2]+1.0);
	...
	else
	{
		...
		CreateCircleAroundpoint(CEM_TDM_Spawns[1][0], CEM_TDM_Spawns[1][1], Circ_Pos[0], Circ_Pos[1], float(random(10)+1));
  		SetPlayerPos(playerid, Circ_Pos[0], Circ_Pos[1], CEM_TDM_Spawns[1][2]+1.0);
  		SetPlayerFacingAngle(playerid, float(random(360)));
		SetPlayerHealth(playerid, 99.0);
		SetPlayerArmour(playerid, 99.0);
		CEM_AliveCount[1]++;
...
stock CEM_TDM_EventLeave(playerid)
{
    CEM_AliveCount[CEM_TDM_PTeam[playerid]]--;
   	format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: Team A : %i - %i : Team B", CEM_AliveCount[0], CEM_AliveCount[1]);
	SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
	...
	if(CEM_AliveCount[0]==0||CEM_AliveCount[1]==0)
	{
	    ...
	    {
	        SendClientMessageToAll(-1, "[EVENT]: Team B has won the event.");
	    }
		else
	    {
	        SendClientMessageToAll(-1, "[EVENT]: Team A has won the event.");
	    }
        CEM_TDM_EventEnd();
	}
	Edit_FoCo_Event_Died(playerid,1);
...
stock CEM_TDM_EventDeath(playerid)
{
	...
	SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
    Edit_FoCo_Event_Died(playerid,1);
    //NOT SURE OF THIS ADDITION BUT INSHALLAH!!!!
	SetPlayerSkin(playerid, GetPVarInt(playerid, "MotelSkin"));
	SetPlayerColor(playerid,GetPVarInt(playerid, "MotelColor"));
	SetPVarInt(playerid, "MotelTeamIssued", 0);
    SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_NORMAL);
...
stock CEM_TDM_EventEnd()
{
	foreach(Player, i)
	{
		if(GetPVarInt(i, "InEvent")==1 && Get_FoCo_Event_Died(i) == 0)
	    {
	    	Edit_FoCo_Event_Died(i, 0);
	    ...
	    Edit_FoCo_Event_Died(i, 0);
	    SetPVarInt(i, "InEvent", 0);
	    SetPVarInt(i, "PlayerStatus", PLAYERSTATUS_NORMAL);
	}
...
stock CEM_CreatRaceCPs()
{
	for(new i = 0; i<CEM_Race_CPCount; i++)
	{
	    ...
	    {
	    	CEM_Race_CPIDs[i]=CreateDynamicRaceCP(CEM_Race_CPType+1, CEM_Race_CP[i][0],CEM_Race_CP[i][1],CEM_Race_CP[i][2],CEM_Race_CP[i][0],CEM_Race_CP[i][1],CEM_Race_CP[i][2],10.0, CEM_RAC_WORLD,CEM_Race_INT,-1,1000000000000.0);
	    }
	    else
	    {
		    CEM_Race_CPIDs[i]=CreateDynamicRaceCP(CEM_Race_CPType, CEM_Race_CP[i][0],CEM_Race_CP[i][1],CEM_Race_CP[i][2],CEM_Race_CP[i+1][0],CEM_Race_CP[i+1][1],CEM_Race_CP[i+1][2],10.0, CEM_RAC_WORLD,CEM_Race_INT,-1,1000000000000.0);
	    }
	}
	foreach(Player, i)
	{
...
stock CEM_Race_EventStart(playerid) //Sumo
{
    ...
    format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %s %s has started a {%06x}Custom Race {%06x}event.  Type /join!", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_NOTICE >>> 8);
    SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
    Edit_Event_InProgress(0);
	Edit_Event_ID(EVENT_CEM);
...
stock CEM_Race_Join(playerid)
{
	...
	{
     	SetPlayerVirtualWorld(playerid, CEM_RAC_WORLD);
        ...
        SetVehicleVirtualWorld(CEM_Race_VEHIDList[CEM_Race_JoinCount], CEM_RAC_WORLD);
...
stock CEM_Race_EventDeath(playerid)
{
	...
	if(CEM_Race_Alive==0)
	{
	    ...
	    {
	        SendClientMessageToAll(COLOR_NOTICE, "[EVENT]: Event ended due to all other racers dying.");
	    }
	    else
	    {
	        SendClientMessageToAll(COLOR_NOTICE, "[EVENT]: Event ended due to all racers dying.");
	    }
	    CEM_Race_EventEnd();
	}
	return 1;
...
stock CEM_Race_EventLeave(playerid)
{
	...
	if(CEM_Race_Alive==0)
	{
        if(CEM_FinishedCount!=0)
	    {
	        SendClientMessageToAll(COLOR_NOTICE, "[EVENT]: Event ended due to all racers finishing the race or quitting it.");
	    }
	    else
	    {
		    SendClientMessageToAll(COLOR_NOTICE, "[EVENT]: Event ended due to all racers quitting.");
	    }
	    CEM_Race_EventEnd();
	}
	event_SpawnPlayer(playerid);
...
stock CEM_FFS_EventStart(playerid) //Sumo
{
    ...
    format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %s %s has started a {%06x}Custom FFA[Sumo] {%06x}event.  Type /join!", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_NOTICE >>> 8);
    SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
    Edit_Event_InProgress(0);
	Edit_Event_ID(EVENT_CEM);
	CEM_FFA_JoinCount=0;
...
stock CEM_FFS_Join(playerid)
{
	...
	{
		SetPlayerVirtualWorld(playerid, CEM_FFA_WORLD);
        ...
        SetVehicleVirtualWorld(CEM_FFA_VEHIDList[CEM_FFA_JoinCount], CEM_FFA_WORLD);
	...
	if(CEM_FFA_Alive<=1)
	{
		foreach(Player, i)
		{
		    if(GetPVarInt(i, "InEvent")==1 && Get_FoCo_Event_Died(i) == 0)
		    {
				if(flag == 1)
				{
					SendAdminMessage(COLOR_WARNING, "[ERROR]: Event is bugged. Please inform Developers.");
                    CEM_FFS_EventEnd();
                    return 1;
				}
		        format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %s has won the Custom-Sumo event.", PlayerName(i));
		        SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
				flag++;
		    }
		}
...
stock CEM_FFDM_EventStart(playerid)
{
    ...
    format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %s %s has started a {%06x}Custom FFA[DeathMatch] {%06x}event.  Type /join!", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_NOTICE >>> 8);
    SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
	Edit_Event_InProgress(0);
	Edit_Event_ID(EVENT_CEM);
	CEM_FFA_JoinCount=0;
...
stock CEM_FFDM_Join(playerid)
{
	...
	new join_random;
	join_random = random(CEM_FFA_SpawnCount);
	SetPlayerPos(playerid, CEM_FFA_Spawns[join_random][0],CEM_FFA_Spawns[join_random][1],CEM_FFA_Spawns[join_random][2]);
	SetPlayerFacingAngle(playerid, CEM_FFA_Spawns[join_random][3]);
	CEM_FFA_JoinCount++;
	if(CEM_FFA_JoinCount==CEM_FFA_SpawnCount)
	{
	    CEM_FFA_JoinCount=0;
...
CMD:cjoin(playerid, params[])
{
	if(GetPVarInt(playerid, "PlayerStatus")==0)
	{
		if(Get_Event_InProgress()!=-1)
		{
		    if(Get_Event_InProgress()==0)
		    {
			    if(Get_FoCo_Event_Died(playerid)==0)
				{
	...
	{
		return SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: You are already at the event, or in a duel.");
	}
	return 1;
}

CMD:cleaveevent(playerid, params[])
{
	if(Get_Event_InProgress()!=-1)
	{
	    if(Get_FoCo_Event_Died(playerid)==0)
	...
	{
	    SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: No event has been started so you cannot leave.");
	}
	return 1;
}

CMD:ceventend(playerid, params[])
{
	if(IsAdmin(playerid, 1)==1)
	{
		if(Get_Event_InProgress()!=-1)
		{
			format(CEM_MSG, sizeof(CEM_MSG), "[EVENT]: %s %s has stopped the custom event.", GetPlayerStatus(playerid), PlayerName(playerid));
			SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
            Edit_Event_InProgress(-1);
            FoCo_Event_Rejoin_Func(0);
			if(Event_Currently_On()==EVENT_CEM)
...
CMD:settdmspawna(playerid, params[])
{
	...
	if(IsAdmin(playerid, 1)==1)
	{
	    if(CEM_RUNNING!=TDM)
	    {
			if(CEM_TDM_LSTSpawner!=playerid)
			{
				//DebugMsg("Entered if new player");
			    CEM_Spawns_Set=-1;
				new CEM_LSTPNAME[24];
				if(CEM_TDM_LSTSpawner==-1)
					CEM_LSTPNAME = "Unknown";
				else
					CEM_LSTPNAME =PlayerName(CEM_TDM_LSTSpawner);
			    format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint of Team-A Created. Last SpawnSet Created by %s",CEM_LSTPNAME);
				SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
				GetPlayerPos(playerid, CEM_TDM_Spawns[0][0], CEM_TDM_Spawns[0][1], CEM_TDM_Spawns[0][2]);
				CEM_TDM_LSTSpawner=playerid;
				//DebugMsg("Finished if(NewPlayer)");
			}
			else
			{
				if(CEM_Spawns_Set==-2)
				{
					//DebugMsg("Entered else if");
				    CEM_Spawns_Set=1;
				    CEM_TDM_INT=GetPlayerInterior(playerid);
					if(GetPlayerVirtualWorld(playerid) != 0)
					    CEM_TDM_WORLD = GetPlayerVirtualWorld(playerid);
					else
					    CEM_TDM_WORLD = 1500;
				    GetPlayerPos(playerid, CEM_TDM_Spawns[0][0], CEM_TDM_Spawns[0][1], CEM_TDM_Spawns[0][2]);
				    format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint of Team-A Created. You can start the event after setting up Skin and Weapons.");
		            SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
		            //DebugMsg("Finished else if");
				}
				else
				{
				    //DebugMsg("Entered else");
				    GetPlayerPos(playerid, CEM_TDM_Spawns[0][0], CEM_TDM_Spawns[0][1], CEM_TDM_Spawns[0][2]);
				    format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint of Team-A Reset. You can start the event after setting up SpawnPoint of Team-B.");
		            SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
		            CEM_Spawns_Set=-1;
		            //DebugMsg("Finished else");
				}
			}
	    }
	    else
	    ...
	    if(CEM_RUNNING!=TDM)
	    {
   			if(CEM_TDM_LSTSpawner!=playerid)
			{
				...
				if(CEM_TDM_LSTSpawner==-1)
					...
					CEM_LSTPNAME =PlayerName(CEM_TDM_LSTSpawner);
			  	format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint of Team-B Created. Last SpawnSet Created by %s", CEM_LSTPNAME);
				SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
				CEM_TDM_LSTSpawner=playerid;
			}
			else
				...
				if(CEM_Spawns_Set==-1)
				{
				    ...
				    CEM_TDM_INT=GetPlayerInterior(playerid);
   					if(GetPlayerVirtualWorld(playerid) != 0)
					    ...
					    CEM_TDM_WORLD = 1500;
				    GetPlayerPos(playerid, CEM_TDM_Spawns[1][0], CEM_TDM_Spawns[1][1], CEM_TDM_Spawns[1][2]);
		   		    format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint of Team-B Created. You can start the event after setting up Skin and Weapons.");
		            SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
				}
				else
				{
				    GetPlayerPos(playerid, CEM_TDM_Spawns[1][0], CEM_TDM_Spawns[1][1], CEM_TDM_Spawns[1][2]);
		    	    format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint of Team-B Reset. You can start the event after setting up SpawnPoint of Team-A.");
		            SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
		            CEM_Spawns_Set=-2;
				}
			}
	...
	if(IsAdmin(playerid, 1)==1)
	{
	    if(CEM_RUNNING!=FFA_SUMO&&CEM_RUNNING!=FFA_DM)
	    {
			if(CEM_FFA_SpawnCount==0)
			{
				...
				if(GetPlayerVirtualWorld(playerid) != 0)
				    CEM_FFA_WORLD = GetPlayerVirtualWorld(playerid);
				else
				    CEM_TDM_WORLD = 1500;
			}
			...
			CEM_FFA_SpawnCount++;
			format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint: %i Created. Use /resetffaspawn to clear it.",CEM_FFA_SpawnCount);
			SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
	    }
	    else
	    {
...
CMD:resetffaspawn(playerid, params[])
{
	if(IsAdmin(playerid, 1)==1)
	{
	    if(CEM_RUNNING!=FFA_SUMO&&CEM_RUNNING!=FFA_DM)
	    {
			...
			SendClientMessage(playerid, COLOR_NOTICE, "[CEM]: SpawnPoints wiped. use /setffaspawn to re-make it.");
	    }
	    else
	    {
	        SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: You cannot edit it while FFA event is running");
	...
	if(IsAdmin(playerid, 1)==1)
	{
	    if(CEM_RUNNING!=RACE)
	    {
  			if(CEM_Race_SpawnCount==0)
			{
				...
				if(GetPlayerVirtualWorld(playerid) != 0)
				    CEM_RAC_WORLD = GetPlayerVirtualWorld(playerid);
				else
				    CEM_RAC_WORLD = 1500;
			}
			...
			CEM_Race_SpawnCount++;
			format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: SpawnPoint: %i Created. Use /resetracespawn to clear it.",CEM_Race_SpawnCount);
			SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
	    }
	    else
	    {
...
CMD:resetracespawn(playerid, params[])
{
	if(IsAdmin(playerid, 1)==1)
	{
	    if(CEM_RUNNING!=RACE)
	    {
			...
			SendClientMessage(playerid, COLOR_NOTICE, "[CEM]: SpawnPoints wiped. use /setracespawn to re-make it.");
...
CMD:setracecp(playerid, params[])
{
	if(IsAdmin(playerid, 1)==1)
	{
	    if(CEM_RUNNING!=RACE)
	    {
			...
			CEM_Race_CPCount++;
			format(CEM_MSG, sizeof(CEM_MSG), "[CEM]: CheckPoint: %i Created. Use /resetracecp to clear it.",CEM_Race_CPCount);
			SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
	    }
	    else
	    {
...
CMD:resetracecp(playerid, params[])
{
	if(IsAdmin(playerid, 1)==1)
	{
	    ...
	    {
			CEM_Race_CPCount=0;
		    SendClientMessage(playerid, COLOR_NOTICE, "[CEM]: CheckPoints wiped. use /setracecp to re-make it.");
	    }
	    else
	    {
	        SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: You cannot edit it while Race event is running.");
...
CMD:chelp(playerid, params[])
{
	if(IsAdmin(playerid, 1)==1)
	{
		SendClientMessage(playerid, COLOR_SYNTAX, "[COMMAND]: /cem - Start/Edit CE | /cjoin - Join CE | /cleaveevent - Leave CE | /ceventend - End CE");
		SendClientMessage(playerid, COLOR_SYNTAX, "[COMMAND]: /settdmspawna - Set TDM Spawn A | /settdmspawnb - Set TDM Spawn B");
		SendClientMessage(playerid, COLOR_SYNTAX, "[COMMAND]: /setffaspawn - Set FFA DM/Sumo Spawn | /resetffaspawn - Reset FFA/Sumo Spawns");
		SendClientMessage(playerid, COLOR_SYNTAX, "[COMMAND]: /setracespawn - Set Race Spawn | /resetracespawn - Reset Race Spawn | /setracecp - /resetracecp");
	}
	return 1;
}
/*******************************************************************************
...
/*******************************************************************************
////////////////////////Main Event Syncer Functions/////////////////////////////
*******************************************************************************/

forward CEM_MJoin(playerid);
public CEM_MJoin(playerid)
{
   cmd_cjoin(playerid, "");
   return 1;
}
forward CEM_MLeave(playerid);
public CEM_MLeave(playerid)
{
   cmd_cleaveevent(playerid, "");
   return 1;
}
forward CEM_MEndEvent(playerid);
public CEM_MEndEvent(playerid)
{
   	cmd_ceventend(playerid, "");
   	return 1;
}
forward CEM_MEventStart(playerid);
public CEM_MEventStart(playerid)
{
	cmd_cem(playerid, "");
	return 1;
}

forward CEM_MAutoJoin(playerid);
public CEM_MAutoJoin(playerid)
{
    CEM_CallFunction(1, playerid);
    return 1;
}
