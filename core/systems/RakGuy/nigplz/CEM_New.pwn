  			...
  			else
  			{
			    ...
			    switch(listitem)
			    {
					...
					case 3:
					{
						...
						{
						    	SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: Some event is already in progress.");
						}
					}
			    }
			}
...
stock CEM_TDM_Join(playerid)
{
	...
	if(CEM_TDM_JoinCount%2==0)
	{
		...
		SetPlayerColor(playerid, CEM_TDM_TEAMA);
  		SetPlayerPos(playerid, CEM_TDM_Spawns[0][0] + (CEM_TDM_JoinCount * 2.0), CEM_TDM_Spawns[0][1] + (y_count * 2.0), CEM_TDM_Spawns[0][2]+1.0);
	...
	else
	{
		...
		SetPlayerPos(playerid, CEM_TDM_Spawns[1][0] + (CEM_TDM_JoinCount * 2.0), CEM_TDM_Spawns[1][1] + (y_count * 2.0), CEM_TDM_Spawns[1][2]+1.0);
	    SetPlayerFacingAngle(playerid, float(random(360)));
		SetPlayerHealth(playerid, 99.0);
		SetPlayerArmour(playerid, 99.0);
		CEM_AliveCount[1]++;
