...
new CEM_TDM_CurrSkinID[MAX_PLAYERS];
	/*************************************************
 					Variables for RACE
	*************************************************/
...
*******************************************************************************/
stock CreateMessage(id)
{
	CEM_MSG="";
	if(id==1)
	{
		format(CEM_MSG, sizeof(CEM_MSG),CEM_Dialogs[id][dialog_content],(CEM_Spawns_Set==1) ? CEM_YN,CEM_Team_Skins[0],CEM_Team_Skins[1],CEM_TDM_Weapons[0][0],CEM_TDM_Weapons[0][1],CEM_TDM_Weapons[1][0],CEM_TDM_Weapons[1][1],CEM_TDM_Weapons[2][0],CEM_TDM_Weapons[2][1],CEM_TDM_Weapons[3][0],CEM_TDM_Weapons[3][1]);
	}
	else if(id==2)
	{
	    format(CEM_MSG, sizeof(CEM_MSG),CEM_Dialogs[id][dialog_content],CEM_Race_SpawnCount,CEM_Race_CPNames[CEM_Race_CPType/3],CEM_Race_CPCount,CEM_Race_VehID);
	}
	else if(id==3)
	{
		format(CEM_MSG, sizeof(CEM_MSG),CEM_Dialogs[id][dialog_content],CEM_FFA_SpawnCount,CEM_FFA_VehID);
	}
	else if(id==FFA_DM)
	{
		format(CEM_MSG, sizeof(CEM_MSG),CEM_Dialogs[id][dialog_content],CEM_FFA_SpawnCount,(CEM_FFA_Rejoin==1) ? CEM_YN,CEM_FFA_Weapons[0][0],CEM_FFA_Weapons[0][1],CEM_FFA_Weapons[1][0],CEM_FFA_Weapons[1][1],CEM_FFA_Weapons[2][0],CEM_FFA_Weapons[2][1],CEM_FFA_Weapons[3][0],CEM_FFA_Weapons[3][1],(CEM_FFA_Armour==true) ? CEM_YN);
	}
	if(CEM_Startable[id-1]==1)
	{
	    format(CEM_MSG, sizeof(CEM_MSG), "%s\n{112233}StartEvent",CEM_MSG);
	}
	return 1;
}
...
stock CEM_TDM_Join(playerid)
{
	...
	SetPlayerVirtualWorld(playerid, 1500);
   	CEM_TDM_CurrSkinID[playerid]=GetPlayerSkin(playerid);
...
stock CEM_TDM_EventLeave(playerid)
{
	...
	SendClientMessageToAll(COLOR_NOTICE, CEM_MSG);
	if(CEM_TDM_CurrSkinID[playerid]!=-1)
	{
	    SetPlayerSkin(playerid, CEM_TDM_CurrSkinID[playerid]);
	    //GiveTeam'sColor
	}
...
stock CEM_TDM_EventEnd()
{
	foreach(Player, i)
	{
		if(GetPVarInt(i, "InEvent")==1)
	    {
			...
			SetPVarInt(i, "InEvent", 0);
			SpawnPlayer(i);
			if(CEM_TDM_CurrSkinID[i]!=-1)
			{
			    SetPlayerSkin(i, CEM_TDM_CurrSkinID[i]);
			    //GiveTeam'sColor
			}
		}
	}
