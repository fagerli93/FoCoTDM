...
stock ProxyCheckLog(data[])
{
	new entry[256];
    format(entry, sizeof(entry), "Date %s | %s\r\n", TimeStamp(), data);
    new File:hFile;
    hFile = fopen("FoCo_Scriptfiles/Logs/pcheck.txt", io_append);
    fwrite(hFile, entry);
    fclose(hFile);
	return 1;
}
...
public OnPlayerProxyCheck(p_sid, response_code, data[])
{
	...
	if(PROXY_PLAYER[pid][Responses] == ONLINE_PROXY_SERVERS)
	{
		...
		for(new i=0; i<PROXY_SITES_NUM ; i++)
		{
		    if(PROXY_PLAYER[pid][proxyresponse][i] == 1)
		    {
				pstrike++;
		    }
		}
		if(pstrike >= 2)
			format(PROXY_MSG, sizeof(PROXY_MSG), "[GUARDIAN]: Server has got reply from all Proxy-Tester Sites for %s(%i). Result: VPN/Proxy[%i/%i].", PlayerName(pid), pid, pstrike, PROXY_SITES_NUM, pid);
		else if(pstrike == 1)
            format(PROXY_MSG, sizeof(PROXY_MSG), "[GUARDIAN]: Server has got reply from all Proxy-Tester Sites for %s(%i). Result: Possible VPN/Proxy[%i/%i].", PlayerName(pid), pid, pstrike, PROXY_SITES_NUM, pid);
		else
		    format(PROXY_MSG, sizeof(PROXY_MSG), "[GUARDIAN]: Server has got reply from all Proxy-Tester Sites for %s(%i). Result: Home Internet[%i/%i].", PlayerName(pid), pid, pstrike, PROXY_SITES_NUM, pid);
