...
#define DR_PICKUPTIMER 60000

#pragma unused Edit_Event_InProgress

new DR_TotLaps;

hook OnGameModeInit()
{
	DR_TotLaps = DR_TotalLaps;
	return 1;
}

CMD:sdrl(playerid, params[])
{
	if(IsAdmin(playerid, 1))
	{
		...
		{
			return SendClientMessage(playerid, COLOR_SYNTAX, "/editlaps [Amount]");
		}
		if(amount <= 0)
		{
			return SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: Amount must be more than 0");
		}
		DR_TotLaps = amount;
	}
	return 1;
...
CMD:setdrlaps(playerid, params[])
{
	return cmd_sdrl(playerid, params);
}

CMD:setdrl(playerid, params[])
{
	return cmd_sdrl(playerid, params);
}

CMD:setdeathracelaps(playerid, params[])
{
	return cmd_sdrl(playerid, params);
}
...
stock DR_CreateCheckPoints()
{
	for(new i=0; i<DR_CPCount; i++)
	{
	    ...
	    if(i!=DR_CPCount-1)
			DR_CheckPIDs[i]=CreateDynamicRaceCP(0, DR_CheckPoints[i][0], DR_CheckPoints[i][1], DR_CheckPoints[i][2], DR_CheckPoints[i+1][0], DR_CheckPoints[i+1][1], DR_CheckPoints[i+1][2], 15.0, DR_VWORLD, -1, -1, 5000.0);
		else
		    DR_CheckPIDs[i]=CreateDynamicRaceCP(1, DR_CheckPoints[i][0], DR_CheckPoints[i][1], DR_CheckPoints[i][2], DR_CheckPoints[0][0], DR_CheckPoints[0][1], DR_CheckPoints[0][2], 15.0, DR_VWORLD, -1, -1, 5000.0);
...
{
	if(_:DR_TextDrawIDs[playerid] != -1)
	{
	    PlayerTextDrawHide(playerid, DR_TextDrawIDs[playerid]);
...
timer RemakePickups[DR_PICKUPTIMER]()
{
    ...
    if(IsPickUpsOn==false)
    {
		...
		foreach(Player, i)
	    {
	        if(Event_Currently_On() == 24 && GetPVarInt(i, "InEvent") == 1)
	        {
...
public DR_EventStart(playerid)
{
    ...
    IsPickUpsOn=false;
    DR_DestroyPickups();
    format(DR_MSG, sizeof(DR_MSG), "[EVENT]: %s %s has started {%06x}DeathRace {%06x}event. Type /join! Price: %d", GetPlayerStatus(playerid), PlayerName(playerid), COLOR_WARNING >>> 8, COLOR_CMDNOTICE >>> 8, TDM_COST);
	SendClientMessageToAll(COLOR_CMDNOTICE, DR_MSG);
	/*
		FoCo Related
	*/
	Edit_Event_InProgress(0);
	Edit_Event_ID(24);		// 24 == DeathRace
	Edit_EventDelay(30);
	// End of FoCo
	DRace_JoinCount=0;
	DRace_Alive=0;
...
stock ResetDRVars(playerid)
{
	...
	DR_PlayerInfo[playerid][DR_Kills]=0;
	DR_PlayerInfo[playerid][DR_Laps]=0;
...
public DR_EventEnd()
{
	...
	if(DR_MostKills>0)
	{
		format(DR_MSG, sizeof(DR_MSG), "[EVENT NOTICE]: %s has killed the most in the Death-Race event. Kills: %i", PlayerName(DR_MostKiller),DR_MostKills);
...
{
	SendClientMessageToAll(COLOR_NOTICE,"[EVENT]: DeathRace is now in progress and can not be joined.");
	foreach(Player, i)
	{
		if(GetPVarInt(i, "InEvent") == 1)
		{
			...
			increment = 0;
			GameTextForPlayer(i, "~R~Event Started!~n~~w~Pickups will be enabled in 30 seconds.", 1000, 3);
		}
	}
 	Edit_Event_InProgress(1);
	return 1;
	...
	if(DR_AntiBug <= 1)
	{
	    ...
	    foreach(Player, i)
	    {
	        if(GetPVarInt(i, "InEvent")==1 && Get_FoCo_Event_Died(i) == 0)
	        {
	        	...
	        	{
		            if(DR_FinishCount!=0)
		            	format(DR_MSG, sizeof(DR_MSG), "[EVENT NOTICE]: %s was the last alive player in the DeathRace and thereby, has won the event(Such a pussy).", PlayerName(i));
					else
	                    format(DR_MSG, sizeof(DR_MSG), "[EVENT NOTICE]: %s won the DeathRace by finishing first!", PlayerName(i));
...
forward DR_OnPlayerDisconnect(playerid);
public DR_OnPlayerDisconnect(playerid)
{
    if(Event_Currently_On()==24)
	{
...
/*****************************Event Call Backs*********************************/
////////////////////////////////////////////////////////////////////////////////
forward Dr_OnPlayerPickUpDynamicPickup(playerid, pickupid);
public Dr_OnPlayerPickUpDynamicPickup(playerid, pickupid)
{
	if(Event_Currently_On()==24)
	{
	    if(GetPVarInt(playerid, "InEvent")==1)
	    {
			if(IsPickUpsOn==true)
			{
				...
				if(DR_PickUpFlag>=0)
				{
					...
					{
					    //ifPlayerIsInEvent
					    if(gettime() - DR_Picks[DR_PickUpFlag][DR_Taken] > DR_PICKUP_TO)
					    {
							...
							DR_Picks[DR_PickUpFlag][DR_Taken]=gettime();
							UpdateDRPText(playerid);
						}
						else
						{
			   				GameTextForPlayer(playerid, "PickUp is rebuilding...~n~Takes 5 seconds.", 3000, 3);
						}
					}
				}
				else
				{
					...
					if(DR_PickUpFlag>=0)
					{
						if(DR_PickUpFlag!=playerid)//DestroyPICKUP
						{
						    ...
						    DR_ExplosionMark(DR_PickUpFlag, 10.0, Pos[0],Pos[1],Pos[2]);
						}
					}
				}
			}
...
forward Dr_OnPlayerExitVehicle(playerid, vehicleid);
public Dr_OnPlayerExitVehicle(playerid, vehicleid)
{
	...
	if(Get_Event_InProgress() !=-1)
	{
		if(checkpointid==DR_CheckPIDs[DR_PlayerInfo[playerid][DR_CheckPoint]])
		{
		    ...
		    if(DR_PlayerInfo[playerid][DR_CheckPoint]==DR_CPCount)
		    {
				...
				if(DR_PlayerInfo[playerid][DR_Laps]==DR_TotLaps)
				{
				    DR_FinishCount++;
				...
				else
				{
					if(DR_PlayerInfo[playerid][DR_Laps]<DR_TotLaps)
					{
					    format(DR_LMSG, sizeof(DR_LMSG), "Lap: ~n~~r~%i ~w~of ~b~%i", DR_PlayerInfo[playerid][DR_Laps],DR_TotLaps);
			...
			{
			    TogglePlayerDynamicRaceCP(playerid, checkpointid, 0);
			    if(DR_PlayerInfo[playerid][DR_Laps]==DR_TotLaps-1 && DR_PlayerInfo[playerid][DR_CheckPoint] == DR_CPCount-1)
			    {
