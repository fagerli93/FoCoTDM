...
#define COLOR_TL 		 0xC46210FF		//Color of 3DTextDraw
...
#define TL_MAXPLoss 	 0.2999	//Maximum P/L allowed to hack, more will Cause Kick.
...
#define TL_FPS_LIMIT	 20		//FPS Limit 20/below gets strike.
#define TL_FPS_SLIMIT	 5		//FPS Strike limit before an admins gets notified..
#define TL_SPAM_LIMIT    5000
...
new TL_AntiSpamRate[MAX_PLAYERS][TL_Codes];
...
hook OnPlayerConnect(playerid)
{
	...
	FPSStrike[playerid] = 0;
	TL_FPS[playerid][0]=0;
	TL_FPS[playerid][1]=0;
	TL_Ping[playerid][0]=0;
	TL_Ping[playerid][1]=0;
	TL_AntiSpamRate[playerid][FPS] = 0;
	TL_AntiSpamRate[playerid][PING] = 0;
	TL_AntiSpamRate[playerid][PACKET] = 0;
...
ptask TL_Updater[1000](playerid)
{
	...
	if(GetPlayerDrunkLevel(playerid)!= TL_FPS[playerid][0] && GetPlayerDrunkLevel(playerid) != 0)
	{
		...
		else
		{
			...
			if(TL_FPS[playerid][0] <= TL_FPS_LIMIT)
			{
				FPSStrike[playerid]++;
				format(TL_MSG, sizeof(TL_MSG), "FPS below 20.~n~Strike(%i/%i)", FPSStrike[playerid], TL_FPS_SLIMIT);
				GameTextForPlayer(playerid, TL_MSG, 1010, 6);
				if(FPSStrike[playerid] % TL_FPS_SLIMIT == 0)
				{
					format(TL_MSG, sizeof(TL_MSG), "[Guardian]: %s(%i) has low FPS.[Recommended Action: Kick/Ask player to Fix it.]", PlayerName(playerid), playerid);
					//FPKLog(TL_MSG);
					if(TL_AntiSpamRate[playerid][FPS] - NetStats_GetConnectedTime(playerid) > TL_SPAM_LIMIT)
					{
						SendAdminMessage(1, TL_MSG);
						TL_AntiSpamRate[playerid][FPS] = NetStats_GetConnectedTime(playerid);
					}	
				}
			}
			else
	...
	if(TL_PKickOverRide[playerid]==false && TL_KickOverRide==false)
	{
		...
		if(TL_TempPing!=0)
		{
			...
			if((TL_TempDiff>=TL_PingJumpLimit || TL_TempDiff<=-TL_PingJumpLimit) && TL_Ping[playerid][1]!=TL_TempPing)
			{
				PingCoolDown[playerid]=TL_PingCoolDown;
				PingStrike[playerid]++;
				GameTextForPlayer(playerid, "~w~Please fix your ping~n~~r~its jumpy.", 1050, 6);
/*Edit*/						///////////Strike-Add///////////////
				...
				{
					format(TL_MSG, sizeof(TL_MSG), "[Guardian]: %s(%i) has Jumpy-Ping.[Recommended Action: Kick/Ask player to Fix it.]", PlayerName(playerid), playerid);
					if(TL_AntiSpamRate[playerid][PING] - NetStats_GetConnectedTime(playerid) > TL_SPAM_LIMIT)
					{
						SendAdminMessage(1, TL_MSG);
						TL_AntiSpamRate[playerid][PING] = NetStats_GetConnectedTime(playerid);
					}	
/*Edit*/            //FPKLog(TL_MSG);
	...
	if(FoCo_Player[playerid][onlinetime]>30)
	{
		...
		if(Temp_PLoss > TL_MAXPLoss)
		{
		    if(Temp_PLoss != TL_PacketLoss[playerid])
		    {
		        if(Temp_PLoss >= TL_PacketLoss[playerid])
		        {
		            if(Temp_PLoss > TL_MAXPLoss && TL_PacketLoss[playerid]<=0.1)
		            {
		                GameTextForPlayer(playerid, "~w~Please avoid moving~n~~r~your packetloss is increasing.", 1010, 6);
...
/*Edit*/                //FPKLog(TL_MSG);
		            ...
		            if(Temp_PLoss - TL_PacketLoss[playerid] >= 0.8)
					...
					if(PacketStrike[playerid]>=M_PacketStrike)
					{
						...
						if(TL_AntiSpamRate[playerid][PACKET] - NetStats_GetConnectedTime(playerid) > TL_SPAM_LIMIT)
						{
							SendAdminMessage(1, TL_MSG);
							TL_AntiSpamRate[playerid][PACKET] = NetStats_GetConnectedTime(playerid);
						}	
/*Edit*/                //FPKLog(TL_MSG);
				...
				else
				{
					...
					if(PacketCoolDown[playerid]==0)
					{
						...
						{
							format(TL_MSG, sizeof(TL_MSG), "[Guardian]: %s(%i) is re-synchronizing.[Recommended Action: Wait and STFU.]", PlayerName(playerid), playerid);
							if(TL_AntiSpamRate[playerid][PACKET] - NetStats_GetConnectedTime(playerid) > TL_SPAM_LIMIT)
							{
								SendAdminMessage(1, TL_MSG);
								TL_AntiSpamRate[playerid][PACKET] = NetStats_GetConnectedTime(playerid);
...
CMD:togtext(playerid, params[])
{
	if(TL_AdminOR==false || IsAdmin(playerid, ACMD_FPTO))
	{
		...
		{
		    SendClientMessage(playerid, COLOR_WARNING, "[ERROR:] Admin has disabled the command for you.");
		}
	}
	else
	{
