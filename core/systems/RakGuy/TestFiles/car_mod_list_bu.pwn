	    ...
	    else
	    {
			...
			else if(strcmp(inputtext, "Checkout", true) == 0)
			{
				...
				for(new i = 1; i < 12; i++)
				{
					...
					{	
						if(TempModSave[playerid][ModPositionNames[i][Mod_Slot]] >= 1000 && CurrentMods[TempModSave[playerid][15]][ModPositionNames[i][Mod_Slot]] < 1000)
						{
					...
					{
						if(TempModSave[playerid][ModPositionNames[i][Mod_Slot]] != 3)
						{
							totprice += ModPositionNames[i][Mod_Price];
						}						
					}
