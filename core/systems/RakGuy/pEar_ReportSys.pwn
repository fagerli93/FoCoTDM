...
{
	new string[56];
	format(string, sizeof(string), "Reached OPD for: %s(%d)", PlayerName(playerid), playerid);
	DebugMsg(string);
	Player_Active_Reports[playerid] = 0;
	for(new i = 0; i < MAX_REPORTS_PER_PLAYER; i++)
	{
		format(string, sizeof(string), "For loop: %i", i);
		DebugMsg(string);
		if(Player_Reports[playerid][i] != -1)
		{
			format(string, sizeof(string), "Pl_Rep[pid][i] != -1, it is: %d", Player_Reports[playerid][i]);
			DebugMsg(string);
			p_reports[Player_Reports[playerid][i]][p_playerid] = -1;
			format(string, sizeof(string), "Set p_reports[%d][pid]: %d", Player_Reports[playerid][i], p_reports[Player_Reports[playerid][i]][p_playerid]);
			DebugMsg(string);	
		}
	...
	for(new i = 0; i < MAX_PLAYERS * MAX_REPORTS_PER_PLAYER; i++)
	{
		format(string, sizeof(string), "Superfor in pos: %d", i);
		DebugMsg(string);
		if(p_reports[i][p_targetid] == playerid)
		{
			format(string, sizeof(string), "p_rep[%d][p_targetid](%d) == playerid(%d)", i, p_reports[i][p_targetid], playerid);
			DebugMsg(string);
			new string[256];
	...
	switch(dialogid)
	{
		...
		case DIALOG_REPORT_HANDLE:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0: AcceptReport(playerid, DialogReport[playerid]);
					case 1: 
					{
...
CMD:myreports(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_REPORT_VIEW_PLAYER, DIALOG_STYLE_TABLIST_HEADERS, "My Reports", string, "Close", "Delete");
	return 1;
}
