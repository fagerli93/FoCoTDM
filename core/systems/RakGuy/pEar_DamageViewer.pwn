...
CMD:takendamage(playerid, params[])
{
	if(IsAdmin(playerid, ACMD_VIEWDAMAGE))
	{
		...
		for(new i = 0; i < MAX_PLAYERS_WATCHING_DMG; i++)
		{
			if(viewDamage[targetid][i] == -1 && pos[0] == -1) //Rak's Edit
			{
				pos[0] = i;
			}
			if(viewingDamage[playerid][i] == -1 && pos[1] == -1)//Rak's Edit
			{
				pos[1] = i;
			}
		}
...
CMD:givendamage(playerid, params[])
{
	if(IsAdmin(playerid, ACMD_VIEWDAMAGE))
	{
		...
		for(new i = 0; i < MAX_PLAYERS_WATCHING_DMG; i++)
		{
			if(GivenDamage[targetid][i] == -1 && pos[0] == -1)
			{
				pos[0] = i;
			}
			if(GivenDamageID[playerid][i] == -1 && pos[1] == -1)
			{
				pos[1] = i;
			}
		}
...
{
	DebugMsg("OPD DamageViewer Start");
	new string[128];
	for(new i = 0; i < MAX_PLAYERS_WATCHING_DMG; i++)
	{
		for(new j = 0; j < MAX_PLAYERS_WATCHING_DMG; j++)
