...
<<<<<<< .mine
			    ...
			    DebugMsg("Set playerstatus = 0 in hook.");
=======
>>>>>>> .r50
...
<<<<<<< .mine
			    SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_NORMAL);
			    DebugMsg("Set playerstatus = 0 in OnPlayerDisconnect");
=======
>>>>>>> .r50
			    CEM_CallFunction(2, playerid);
			}
...
{
	DebugMsg("Set playerstatus = 0 OnPlayerConnect");
...
<<<<<<< .mine
	...
	{
	    SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_NORMAL);
	    DebugMsg("Set playerstatus = 0 on CEM_CallFunction");
	}
=======
>>>>>>> .r50
	/*************************************************
...
stock CEM_TDM_Join(playerid)
{
	...
	new dstring[56];
	format(dstring, sizeof(dstring), "Playerstatus(%d): %d", playerid, GetPVarInt(playerid, "PlayerStatus"));
	DebugMsg(dstring);
	if(CEM_TDM_JoinCount%2==0)
	{
		...
		SetPlayerColor(playerid, CEM_TDM_TEAMA);
  		SetPlayerPos(playerid, CEM_TDM_Spawns[0][0]+(4.0*floatcos(90.0+(CEM_TDM_Spawns[0][0]+CEM_AliveCount[0]*1.8), degrees)), CEM_TDM_Spawns[0][1]+(4.0*floatsin(90.0-(CEM_TDM_Spawns[0][1]+CEM_AliveCount[0]*1.8), degrees)), CEM_TDM_Spawns[0][2]+1.0);
	...
	else
	{
		...
		SetPlayerPos(playerid, CEM_TDM_Spawns[1][0]+(4.0*floatcos(90.0+(CEM_TDM_Spawns[1][0]+CEM_AliveCount[1]*1.8), degrees)), CEM_TDM_Spawns[1][1]+(4.0*floatsin(90.0-(CEM_TDM_Spawns[1][1]+CEM_AliveCount[1]*1.8), degrees)), CEM_TDM_Spawns[1][2]+1.0);
	    SetPlayerFacingAngle(playerid, float(random(360)));
		SetPlayerHealth(playerid, 99.0);
		SetPlayerArmour(playerid, 99.0);
		CEM_AliveCount[1]++;
...
stock CEM_TDM_EventDeath(playerid)
{
    ...
    Edit_FoCo_Event_Died(playerid,1);
    SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_NORMAL);
    DebugMsg("Set playerstatus = 0 CEM_TDM_EventDeath");
	if(CEM_AliveCount[0]==0||CEM_AliveCount[1]==0)
	{
	    if(CEM_AliveCount[0]==0)
	    {
...
stock CEM_TDM_EventEnd()
{
	foreach(Player, i)
	{
	    ...
	    DebugMsg("Set playrstatus = 0 CEM_TDM_EVENTEND");
	}
...
stock CEM_Race_Join(playerid)
{
	if(CEM_Race_JoinCount!=CEM_Race_SpawnCount)
	{
		...
		SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_INEVENT);
		new dstring[56];
		format(dstring, sizeof(dstring), "Set PS = %d for PID: %d", GetPVarInt(playerid, "PlayerStatus"), playerid);
		DebugMsg(dstring);
		SetPVarInt(playerid, "InEvent",1);
	}
	else
...
stock CEM_Race_EventDeath(playerid)
{
	...
	SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_NORMAL);
	DebugMsg("Set playerstatus = 0 CEM_RACE_EventDeath");
	RemovePlayerFromVehicle(playerid);
...
stock CEM_Race_EventLeave(playerid)
{
	...
	SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_NORMAL);
	DebugMsg("Set playerstatus = 0 in CEM_Race_EventLeave");
	RemovePlayerFromVehicle(playerid);
...
stock CEM_Race_EventEnd()
{
	...
	foreach(Player, i)
    {
	    ...
	    SetPVarInt(i, "InEvent", 0);
	    SetPVarInt(i, "PlayerStatus", PLAYERSTATUS_NORMAL);
	    DebugMsg("Set playerstatus = 0 on CEm_race_Eventend");
    }
...
stock CEM_FFS_Join(playerid)
{
	if(CEM_FFA_JoinCount!=CEM_FFA_SpawnCount)
	{
		...
		SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_INEVENT);
		new dstring[56];
		format(dstring, sizeof(dstring), "Set ps: %d for PID: %d in CEM_FFS_join", GetPVarInt(playerid, "PlayerStatus"), playerid);
	}
	else
	{
...
{
    Edit_FoCo_Event_Died(playerid,1);
   	DebugMsg("Set playerstatus = 0 in cem_ffs_eventdeath");
	CEM_FFA_Alive--;
	if(CEM_FFA_Alive==1)
	{
		foreach(Player, i)
...
{
	Edit_FoCo_Event_Died(playerid,1);
   	DebugMsg("Set playerstatus = 0 in CEM_ffs_eventleave");
	CEM_FFA_Alive--;
	if(CEM_FFA_Alive<=1)
	{
		foreach(Player, i)
...
stock CEM_FFS_EventEnd()
{
	foreach(Player, i)
    {
	    ...
	    SetPVarInt(i, "InEvent", 0);
	    SetPVarInt(i, "PlayerStatus", PLAYERSTATUS_NORMAL);
	    DebugMsg("Set ps = 0 in CEM_ffs_eventend");
    }
...
stock CEM_FFDM_Join(playerid)
{
	...
	SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_INEVENT);
	new dstring[56];
	format(dstring, sizeof(dstring), "Set ps: %d for PID: %d in CEM_ffdm_join", GetPVarInt(playerid, "PlayerStatus"), playerid);
	DebugMsg(dstring);
	SetPlayerInterior(playerid, CEM_FFA_INT);
	SetPlayerVirtualWorld(playerid, 1500);
	for(new i=0; i<4; i++)
...
stock CEM_FFDM_EventDeath(playerid)
{
	...
	DebugMsg("Set ps = 0 in CEM_ffdm_eventdeath");
	return 1;
}
stock CEM_FFDM_EventLeave(playerid)
{
	...
	DebugMsg("Set ps = 0 in CEM_ffdm_event_leave");
	SpawnPlayer(playerid);
}
stock CEM_FFDM_EventEnd()
{
	foreach(Player, i)
	{
	    ...
	    SetPVarInt(i, "InEvent", 0);
	    SetPVarInt(i, "PlayerStatus", PLAYERSTATUS_NORMAL);
	    DebugMsg("Set ps = 0 in CEM_ffdm_eventend");
	}
	CEM_RUNNING=NILVALUE;
	Edit_Event_InProgress(-1);
	return 1;
			    ...
			    if(Get_FoCo_Event_Died(playerid)==0)
				{
					if(GetPVarInt(playerid, "InEvent")==0)
					{
						if(Event_Currently_On()==EVENT_CEM)
						{
						    SetPVarInt(playerid, "PlayerStatus", PLAYERSTATUS_INEVENT);
						    new dstring[56];
						    format(dstring, sizeof(dstring), "Set Ps: %d for %d in cjoin", GetPVarInt(playerid, "PlayerStatus"), playerid);
						    DebugMsg(dstring);
	...
	if(Get_Event_InProgress()!=-1)
	{
	    if(Get_FoCo_Event_Died(playerid)==0)
	    {
			if(GetPVarInt(playerid, "InEvent")==1)
			{
				if(Event_Currently_On()==EVENT_CEM)
				{
				    DebugMsg("Set ps = 0 in cleaveevent");
...
{
	SendClientMessageToAll(-1, "DEBUG]:Entered /settdmspawna");
	if(IsAdmin(playerid, 1)==1)
	{
	    if(CEM_RUNNING!=TDM)
	    {
			if(CEM_TDM_LSTSpawner!=playerid)
			{
			    SendClientMessageToAll(-1, "DEBUG]:Entered if(NewPlayer)");
			    CEM_Spawns_Set=-1;
			    SendClientMessageToAll(-1, "DEBUG]:Entering format");
				new CEM_LSTPNAME[24];
				if(CEM_TDM_LSTSpawner==-1)
					CEM_LSTPNAME = "Unknown";
				else
					CEM_LSTPNAME =PlayerName(CEM_TDM_LSTSpawner);
			    format(CEM_MSG, sizeof(CEM_MSG), "[CEM_NOTICE]: SpawnPoint of Team-A Created. Last SpawnSet Created by %s",CEM_LSTPNAME);
                SendClientMessageToAll(-1, "DEBUG]:Finished format");
				SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
				GetPlayerPos(playerid, CEM_TDM_Spawns[0][0], CEM_TDM_Spawns[0][1], CEM_TDM_Spawns[0][2]);
				CEM_TDM_LSTSpawner=playerid;
				SendClientMessageToAll(-1, "DEBUG]:Finished if(NewPlayer)");
			}
			else
			{
				if(CEM_Spawns_Set==-2)
				{
				    SendClientMessageToAll(-1, "DEBUG]:Entered else: if:");
				    CEM_Spawns_Set=1;
		            ...
		            SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
		            SendClientMessageToAll(-1, "DEBUG]:Finished else: if:");
				}
				else
				{
				    SendClientMessageToAll(-1, "DEBUG]:Entered else: else:");
				    GetPlayerPos(playerid, CEM_TDM_Spawns[0][0], CEM_TDM_Spawns[0][1], CEM_TDM_Spawns[0][2]);
		            ...
		            SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
		            CEM_Spawns_Set=-1;
		            SendClientMessageToAll(-1, "DEBUG]:finished else: else:");
				}
			}
	    }
	    else
	    ...
	    if(CEM_RUNNING!=TDM)
	    {
   			if(CEM_TDM_LSTSpawner!=playerid)
			{
				...
				if(CEM_TDM_LSTSpawner==-1)
					...
					CEM_LSTPNAME =PlayerName(CEM_TDM_LSTSpawner);
			  	format(CEM_MSG, sizeof(CEM_MSG), "[CEM_NOTICE]: SpawnPoint of Team-B Created. Last SpawnSet Created by %s", CEM_LSTPNAME);
				SendClientMessage(playerid, COLOR_NOTICE, CEM_MSG);
				CEM_TDM_LSTSpawner=playerid;
			}
			else
