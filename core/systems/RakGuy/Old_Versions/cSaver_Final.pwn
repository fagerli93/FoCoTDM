#include <YSI\y_hooks>
//#include <YSI\y_ini>//Incase..

/****************************NOTE FOR PEAR**************************************
Check the 119, 132, 138, 139. Edit AC n Shit.. Give money command etc.. DO IT!!!
*******************************************************************************/
...
{
	//Set 1: Day 1-9
	{-2,-2,-1,-1,5000,20000,5000,1000,1000,20000,10000,20000,1000,10000,5000,
...
stock GivePlayerChristmasGift(playerid)
{
	...
	format(GMSG, sizeof(GMSG), "[NOTICE]: You have unpacked %s$ from the Christmas tree.[GiftID: %i]", GiftName, cTotalCount[1]);
...
CMD:getgift(playerid, params[])
{
	...
	if(Gift_Taken[playerid]==false)
	{
		...
		if(Range<GIFT_RADIUS)
		{
            ...
            cPlayerJoinDet[playerid][0]++;
			new Days[3];
			getdate(Days[0], Days[1], Days[2]);
			format(GMSG, sizeof(GMSG), "%i %i",cPlayerJoinDet[playerid][0],Days[2]);
			new INI:cFile = INI_Open(CHRISTMAS_LOGIN);
			INI_WriteString(cFile, PlayerName(playerid), GMSG);
			INI_Close(cFile);
			Gift_Taken[playerid]=true;
            return 1;
		}
		else
		{
			...
			SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: You need to be near a Christmas tree in order to use this command.");
...
CMD:claimgift(playerid, params[])
{
	...
	else
	{
	    if(vGift[playerid]>400 && vGift[playerid]<609)
	    {
			...
			format(GMSG, sizeof(GMSG), "[NOTICE]: %s has claimed his %s from his daily Christmas gift. It is powerful, but not invincible!", PlayerName(playerid), vehName);
			SendClientMessageToAll(COLOR_GLOBALNOTICE, GMSG);
            vGift[playerid]=0;
			return SendClientMessage(playerid, COLOR_SYNTAX, "[NOTICE]: You can get vehicle-jacked and in that case it is your fault!");
	    }
	    else
	    {
