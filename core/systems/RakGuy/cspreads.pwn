...
#define NS_CMDLMT 1
#define DIALOG_NS DIALOG_NO_RESPONSE
...
new BullCount[MAX_PLAYERS],
	SRecoilStrike[MAX_PLAYERS],
	SRecoilWarn[MAX_PLAYERS];
...
hook OnPlayerConnect(playerid)
{
	...
	BullCount[playerid]=0;
	SRecoilStrike[playerid]=0;
	SRecoilWarn[playerid]=0;
}
	...
	if(NS_Toggled==false)
	{
		if(NoSpreadTest[playerid]==true)
		{
			if(weaponid==24)
			{
				...
				if(8.880 - BulletDistance <0.8 && 8.880 - BulletDistance > -0.8)
				{
					if(TempNSTime<2)
					{
					    SRecoilStrike[playerid]++;
					    if(SRecoilStrike[playerid]%3 == 0)
					    {
					        SRecoilWarn[playerid]++;
							if(SRecoilWarn[playerid] % 3 == 0)
					        {
	 							format(NS_MSG, sizeof(NS_MSG), "%s (%i) is possibly using Renko(SemiRecoil).", PlayerName(playerid), playerid);
			                    AntiCheatMessage(NS_MSG);
					        }
					    }
					}
				...
				NSLOG[playerid][BullCount[playerid]%15]=BulletDistance;
				BullCount[playerid]++;
