...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(AllowMapTP[playerid]==2)
	{
		if(newkeys&KEY_YES)
		{
			...
			format(TP_MSG, sizeof(TP_MSG), "AdmCmd(%i): %s %s has used Map-Teleport to TP himself to %s.", CMDLMT_MPTP, GetPlayerStatus(playerid), PlayerName(playerid), MTP_2DLocation);
