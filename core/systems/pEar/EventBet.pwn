...
public Event_Bet_End(win_team)
{
	...
	if(Team_Bet == 1)
	{
	    ...
	    if(win_team == 0)
	    {
	        foreach(Player, i)
	        {
	            if(GetPVarInt(i, "TeamBet") == 0)
	            {
	                ...
	                Percentage_F = ((Float:EventBet[i] / Float:Team1_Bets) * 100.0);
					new Percentage = floatround(Percentage_F, floatround_ceil);
	    ...
	    else
	    {
     		foreach(Player, i)
	        {
	            ...
	            {
	                new Float:Percentage_F;
	                Percentage_F = ((Float:EventBet[i] / Float:Team2_Bets) * 100.0);
					new Percentage = floatround(Percentage_F, floatround_ceil);
	                new Float:Money_Won_F;
	...
	else
	{
		...
		foreach(Player, i)
		{
		    ...
		    {
      			new Float:Percentage_F;
  				Percentage_F = ((Float:EventBet[i] / Float:Total_ID_Bets[win_team]) * 100.0);
				new Percentage = floatround(Percentage_F, floatround_floor);
  				new Float:Money_Won_F;
  				new Float:Money_Tot;
  				Money_Tot = (Float:Total_Event_Bets - Float:Total_ID_Bets[win_team]);
  				Money_Won_F = (((Float:Money_Tot / 100.0) * Float:Percentage) * Float:FOCO_EVENT_BET_COST);
  				new Money_Won = floatround(Money_Won_F, floatround_floor);
				if(Money_Won < 0)
				{
					return 1;
...
CMD:cancelebet(playerid, params[])
{
	...
	SendClientMessage(playerid, COLOR_WARNING, "[ERROR]: You don't have a valid bet to cancel!");
	return 1;
