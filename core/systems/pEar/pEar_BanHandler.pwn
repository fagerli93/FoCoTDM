...
/*
MOVED THIS TO THE MAIN CALLBACK 
...
public ban_handler(playerid)
{
	...
	{
		AKickPlayer(-1, playerid, "Invalid IP connection", 1);
	}
    ipstring[playerid] = IP;
	IP_Ranges[0] = IP;
	for(i = 1; i < 3; i++)
	{
...
public BanPlayer(playerid, targetid, reason[], isPublic)
{
	...
	if(gPlayerLogged[targetid] == 1 || playerid != -1)
	{
...
public SQL_AdminRecordInsert(playerid, targetid, actiontype, reason[])
{
	...
	if(playerid != -1)
	{
		format(string, sizeof(string), "INSERT INTO `FoCo_AdminRecords` (`user`, `admin`, `actiontype`, `reason`, `date`) VALUES ('%d', '%s', '%d', '%s', '%s')", FoCo_Player[targetid][id], PlayerName(playerid), actiontype, reason, TimeStamp());
	...
	if(IsAdmin(playerid, ACMD_BANIP))
	{
		...
		if(sscanf(params, "p<.>iiiis[128]", IPADD[0], IPADD[1], IPADD[2], IPADD[3], reason))
		{
			if(!sscanf(params, "p<.>iiis[128]", IPADD[0], IPADD[1], IPADD[2], reason))
			{
				...
				format(string, sizeof(string), "INSERT INTO `FoCo_Bans` (`ib_ip`, `ib_reason`, `ib_admin`, `ib_rangeban`) VALUES ('%d.%d.%d', '%s', '%s', '1')", IPADD[0], IPADD[1], IPADD[2], reason, PlayerName(playerid));
				mysql_query(string);
				return 1;
			}
			if(!sscanf(params, "p<.>iis[128]", IPADD[0], IPADD[1], reason))
			{
				...
				format(string, sizeof(string), "INSERT INTO `FoCo_Bans` (`ib_ip`, `ib_reason`, `ib_admin`, `ib_rangeban`) VALUES ('%d.%d', '%s', '%s', '1')", IPADD[0], IPADD[1], reason, PlayerName(playerid));
				mysql_query(string);
				return 1;
			}
	...
	switch(dialogid)
	{
		...
		case DIALOG_TEMPBANIP_CONFIRM:
		{
			if(response)
			{
				...
				if(DialogVarBanIP[playerid][IP_3] == -1)
				{
					...
					format(query, sizeof(query), "INSERT INTO `FoCo_Bans` (`ib_ip`, `ib_reason`, `ib_tempbanned`, `ib_unbantime`, `ib_admin`, `ib_rangeban`) VALUES ('%d.%d.%d.%d', '%s', '1', '%d', '%s', '1')", DialogVarBanIP[playerid][IP_1], DialogVarBanIP[playerid][IP_2], DialogVarBanIP[playerid][REASON], unbantime, PlayerName(playerid));
				}
				else if(DialogVarBanIP[playerid][IP_4] == -1)
				{
					...
					format(query, sizeof(query), "INSERT INTO `FoCo_Bans` (`ib_ip`, `ib_reason`, `ib_tempbanned`, `ib_unbantime`, `ib_admin`, `ib_rangeban`) VALUES ('%d.%d.%d', '%s', '1', '%d', '%s', '1')", DialogVarBanIP[playerid][IP_1], DialogVarBanIP[playerid][IP_2], DialogVarBanIP[playerid][IP_3], DialogVarBanIP[playerid][REASON], unbantime, PlayerName(playerid));
				}	
				else
				{
		...
		case DIALOG_TEMPBAN_CONFIRM:
		{
			if(response)
			{
				...
				mysql_query(query, MYSQL_THREAD_TEMPBAN, playerid, con);
...
forward ban_OnQueryFinish(query[], resultid, extraid, connectionHandle);
public ban_OnQueryFinish(query[], resultid, extraid, connectionHandle)
{
	switch(resultid)
	{
		case MYSQL_THREAD_UNBANTEMP: return 1;
		case MYSQL_THREAD_BANIP: return 1;
		case MYSQL_THREAD_TEMPBAN: return 1;
		// ---
		
		case MYSQL_THREAD_BAN_HANDLER:
		{
			mysql_store_result();
			if(mysql_num_rows() > 0)
			{
				new resultline[512], string[256];
				new ib_id, ib_ip[20], ib_unbanunix, ib_banned, ib_tempbanned;
				while(mysql_fetch_row_format(resultline))
				{
					sscanf(resultline, "p<|>ds[20]ddd", ib_id, ib_ip, ib_unbanunix, ib_banned, ib_tempbanned);
					// Is banned, but might be a tempban, so possibly unban?
					...
					if(ib_banned == 1)
					{
						...
						if(ib_tempbanned == 1)
						{
							if(gettime() >= ib_unbanunix)
							{
								// Should be unbanned..
								format(string, sizeof(string), "UPDATE `FoCo_Bans` SET `ib_banned`='0' WHERE `ib_id`='%d'", ib_id);
								mysql_query(string, MYSQL_THREAD_UNBANTEMP, extraid, con);
								continue;
							}
							else
							{
								ShowPlayerDialog(extraid, -1, DIALOG_STYLE_MSGBOX, "", "", "", "");
								if(extraid != INVALID_PLAYER_ID)
								{
									format(string, sizeof(string), "AdmCmd(%d): The Guardian has kicked %s(%d), Reason: Connecting with a banned IP: %s", ACMD_KICK, PlayerName(extraid), extraid, ib_ip);
									IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
									IRC_GroupSay(gEcho, IRC_FOCO_ECHO, string);
									SendAdminMessage(1, string);
									CheatLog(string);
									SendClientMessage(extraid, COLOR_RED, "You are banned from this server.");
									SQL_AdminRecordInsert(-1, extraid, 2, "Connecting with a banned IP");
			...
			continue_OnPlayerConnect(extraid);
			mysql_free_result();
			return 1;
		}
		case MYSQL_THREAD_UNBANIP:
		{
			new string[128];
			mysql_store_result();
			if(mysql_affected_rows() == 0)
			{
				return SendClientMessage(extraid, COLOR_WARNING, "[ERROR]: Could not find and unban the given IP!");
			}
			else
			{
				if(DialogVarBanIP[extraid][IP_3] == -1 || DialogVarBanIP[extraid][IP_3] == 255)
				{
					format(string, sizeof(string), "AdmCmd(%d): %s %s has unbanned the IP-Range %d.%d",ACMD_BANIP, GetPlayerStatus(extraid), PlayerName(extraid), DialogVarBanIP[extraid][IP_1], DialogVarBanIP[extraid][IP_2]);
					SendAdminMessage(ACMD_BANIP,string);
					AdminLog(string);
					IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
					IRC_GroupSay(gEcho, IRC_FOCO_ECHO, string);
				}
				else if(DialogVarBanIP[extraid][IP_4] == -1 || DialogVarBanIP[extraid][IP_4] == 255)
				{
					format(string, sizeof(string), "AdmCmd(%d): %s %s has unbanned the IP-Range %d.%d.%d",ACMD_BANIP, GetPlayerStatus(extraid), PlayerName(extraid), DialogVarBanIP[extraid][IP_1], DialogVarBanIP[extraid][IP_2], DialogVarBanIP[extraid][IP_3]);
					SendAdminMessage(ACMD_BANIP,string);
					AdminLog(string);
					IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
					IRC_GroupSay(gEcho, IRC_FOCO_ECHO, string);
				...
				{
					format(string, sizeof(string), "AdmCmd(%d): %s %s has unbanned IP %d.%d.%d.%d",ACMD_BANIP, GetPlayerStatus(extraid), PlayerName(extraid), DialogVarBanIP[extraid][IP_1], DialogVarBanIP[extraid][IP_2], DialogVarBanIP[extraid][IP_3], DialogVarBanIP[extraid][IP_4]);
					SendAdminMessage(ACMD_BANIP,string);
					AdminLog(string);
					IRC_GroupSay(gLeads, IRC_FOCO_LEADS, string);
					IRC_GroupSay(gEcho, IRC_FOCO_ECHO, string);
