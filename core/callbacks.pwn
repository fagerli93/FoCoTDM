...
/* User successfully logged in and NOT (temp)banned. */
forward OnPlayerLoginSuccess(playerid);
public OnPlayerLoginSuccess(playerid) {
	...
	LoadPlayerStatsInfo(playerid);
	//LoadDuelsCount(playerid);
	FirstLoadClasses(playerid);
	FoCo_Connection(playerid);
	if(FoCo_Player[playerid][donation] > 0)
	{
	    new dquery[256];
		format(dquery, sizeof(dquery), "SELECT * FROM FoCo_Donations WHERE don_id ='%d'",FoCo_Player[playerid][donation]);
		mysql_query(dquery, MYSQL_THREAD_DONATIONS, playerid, con);
	}
	ARecord_OnPlayerLogin(playerid);
	JailEvade_OnPlayerLogin(playerid);
	return 1;
}

public OnGameModeInit()
{
    SetGameModeText("FoCo TDM V4 | Rev: 1001");
	AntiDeAMX();
	Saving_Connect();
	CreateObjects();
	...
	LoadScript(); //Vehicle + Pickup + Turf
	/*
	* Textdraw Creation
	*/
	...
	CurrLeader = TextDrawCreate(283.000000, 400.000000, "Current Leader");
	TextDrawAlignment(CurrLeader, 3);
	TextDrawBackgroundColor(CurrLeader, 255);
	TextDrawFont(CurrLeader, 3);
	TextDrawLetterSize(CurrLeader, 0.500000, 1.000000);
	TextDrawColor(CurrLeader, -1);
	TextDrawSetOutline(CurrLeader, 1);
	TextDrawSetProportional(CurrLeader, 1);

	CurrLeaderName = TextDrawCreate(291.000000, 414.000000, " ");
	TextDrawAlignment(CurrLeaderName, 3);
	TextDrawBackgroundColor(CurrLeaderName, 255);
	TextDrawFont(CurrLeaderName, 3);
	TextDrawLetterSize(CurrLeaderName, 0.400000, 0.899999);
	TextDrawColor(CurrLeaderName, 16777215);
	TextDrawSetOutline(CurrLeaderName, 1);
	TextDrawSetProportional(CurrLeaderName, 1);
...
public OnGameModeExit()
{
	foreach(Player, i)
	{
		TextDrawHideForAll(CurrLeader);
		TextDrawDestroy(CurrLeader);
		TextDrawHideForAll(CurrLeaderName);
		TextDrawDestroy(CurrLeaderName);
...
public OnPlayerConnect(playerid)
{
	...
	AntiDoubleDeath[playerid] = true;
	LastShotAtPlayer[playerid][SAD_Shooter] = INVALID_PLAYER_ID;
	LastShotAtPlayer[playerid][SAD_Time] = 0;
	LastShotAtPlayer[playerid][SAD_Reason] = 1;
	#endif
	SQL_Loaded[playerid][sql_data] = false;
	SQL_Loaded[playerid][sql_pstats] = false;
	SQL_Loaded[playerid][sql_achievements] = false;
    //END_OF_SSHP_REMOVALS
...
{
    FoCo_Player[playerid][online] = 0;
    #if defined SERVERSIDED_HP
	LastShotAtPlayer[playerid][SAD_Shooter] = INVALID_PLAYER_ID;
	LastShotAtPlayer[playerid][SAD_Time] = 0;
	LastShotAtPlayer[playerid][SAD_Reason] = 1;
	FailedLoginAttempts[playerid] = 0;
	AntiDoubleDeath[playerid] = false;
	#endif
	new string[128];
    FoCo_Disconnection(playerid);
    /* This saves the data, just has to run through UCP data first */
    UCP_Check_Update(playerid);
	//DataSave(playerid);
    nullvip(playerid);
    specHP[playerid] = -1;
	specArmour[playerid] = -1;
	...
	if(gPlayerLogged[playerid] == 1)
    {
		...
		if(SQL_Loaded[playerid][sql_data] == true)
			DataSave(playerid);
		if(SQL_Loaded[playerid][sql_achievements] == true)
			AchievementSave(playerid);
		//DuelSave(playerid);
		if(SQL_Loaded[playerid][sql_pstats] == true)
...
public OnPlayerSpawn(playerid)
{
    //SSHP_REMOVALS
    #if defined SERVERSIDED_HP
	SS_DeadPlayer[playerid] = false; //SSHP
	KnifeBugged[playerid] = false;
	LastShotAtPlayer[playerid][SAD_Shooter] = INVALID_PLAYER_ID;
	LastShotAtPlayer[playerid][SAD_Time] = 0;
	LastShotAtPlayer[playerid][SAD_Reason] = 0;
	SetPlayerTeam(playerid, 1); // SSHP
	AntiDoubleDeath[playerid] = false;
	#endif
	//END_OF_SSHP_REMOVALS
	...
	/*          REMOVED THIS TO FUCK TEAM ERROR
...
////////////SSHP BEGIN/////////////////////
    #if defined SERVERSIDED_HP
	if(AntiDoubleDeath[playerid] == true)
	    return 1;
    AntiDoubleDeath[playerid] = true;
	if(killerid == INVALID_PLAYER_ID && LastShotAtPlayer[playerid][SAD_Shooter] != INVALID_PLAYER_ID/* && reason == 54*/)
	{
		if(IsPlayerConnected(LastShotAtPlayer[playerid][SAD_Shooter]))
		{
			if((NetStats_GetConnectedTime(playerid) - LastShotAtPlayer[playerid][SAD_Time] < SHOT_STACK_RESET))
		    {
		        killerid = LastShotAtPlayer[playerid][SAD_Shooter];
		        reason = LastShotAtPlayer[playerid][SAD_Reason];
		    }
		}
	}
	#endif
////////////SSHP END/////////////////////
	...
	if(killerid != INVALID_PLAYER_ID)
	{
		if(GetPVarInt(killerid, "PlayerStatus") == PLAYERSTATUS_NORMAL)           // Team-killing will only show when not in event, duel etc.
		{
			if(FoCo_Team[playerid] == FoCo_Team[killerid])
			{
			    if(GetPVarInt(playerid, "DuelException") != 1 && IsDead[playerid] == false)
			    {
					...
					{
						if(AdminLvl(killerid) < 1 && IsTrialAdmin(killerid) < 1)
						{
						    new string[256];
						...
						{
						    GameTextForPlayer(killerid, "~r~Bitch stop TeamKilling!",3000, 5);
						}
					}
					else
					{
		...
		if(reason == 50)
		{
		    if(GetPlayerVehicleID(killerid) != 0)
		    {
				...
				{
				    AKickPlayer(-1, killerid, "Heliblading", 1);
				}
			}
		}
		if(GetPVarInt(killerid, "PlayerStatus") == PLAYERSTATUS_INEVENT  && IsDead[playerid] == false)
		{
		    if(Event_Currently_On() == MADDOGG || Event_Currently_On() == BIGSMOKE || Event_Currently_On() == MINIGUN || Event_Currently_On() == BRAWL || Event_Currently_On() == HYDRA || Event_Currently_On() == LOD || Event_Currently_On() == HOTLAVA || (Event_Currently_On() == CEM && CEM_RUNNING == FFA_DM) || Event_Currently_On() == GUNGAME || Event_Currently_On() == ONESHOTKILL)
		    {
		        IsLastKillTK[killerid] = -1;
		    }
		    else
		    {	
		        if(GetPVarInt(playerid, "MotelTeamIssued") == GetPVarInt(killerid, "MotelTeamIssued") && GetPVarInt(killerid, "MotelTeamIssued") != 0)
		        {
		            FoCo_Playerstats[killerid][kills] -= (IsLastKillTK[killerid]*2) + 1;
					GameTextForPlayer(killerid, "~r~Team Killing~n~Is Not Allowed!",3000,5);
		...
		{
		    KA_GivePlayerAssistKill(playerid, killerid);
		}
		#endif
		...
		if(GetPVarInt(playerid,"PlayerStatus") != 2)
		{
			...
			if(DeathCamTimer[playerid] == Spec_TimerFix)
			{
				...
				//DEATH_SPAWN_TIME is defined @defines.pwn
				spawnSeconds[playerid]=DEATH_SPAWN_TIME;
	...
	hs_OnPlayerDeath(playerid, killerid, reason);
	a_fkOnPlayerDeath(playerid, killerid, reason);
	#if defined RAKGUY_SPAWNKILL
		SpawnKill_OnPlayerDeath(playerid, killerid, reason);
	#endif
	...
	if(IsDead[playerid]==true)
	{
	    ...
	    {
	        ABanPlayer(-1, playerid, "FakeDeath", 1);
	    }
	    return 1;
	}
	else
...
public OnPlayerText(playerid, text[])
{
	...
	ChatLog(log);
	new sstrval = SearchString(text);
	if(sstrval == 1 && AdminLvl(playerid) < 1)
	{
		new ircMsg[256];
		...
		SendClientMessage(playerid, COLOR_WARNING, "[WARNING]: You are not allowed to mention such word/names. Spamming will get you banned.");
		AdvStrike[playerid]++;
		if(AdvStrike[playerid] == 5)
		{
		    ABanPlayer(-1, playerid, "Spamming Banned Word", 1);
		}
		CheatLog(ircMsg);
	}
	else if(sstrval == 2 &&  AdminLvl(playerid) < 1)
	{
		new ircMsg[256];
		format(ircMsg, sizeof(ircMsg), "[CHAT IGNORED]: %s[%d]: %s.", PlayerName(playerid), playerid, text);
		IRC_GroupSay(gLeads, IRC_FOCO_LEADS, ircMsg);
		SendAdminMessage(1, ircMsg);
		SendClientMessage(playerid, COLOR_WARNING, "[WARNING]: You are not allowed to mention such word/names.");
        CheatLog(ircMsg);
	}
	else
	{
...
public OnPlayerPrivmsg(playerid, recieverid, text[])
{
    ...
    if(mutedPlayers[playerid][muted] == 0)
    {
		if(GetPVarInt(recieverid, "PMBLOCK") == 1 && FoCo_Player[playerid][admin] < 1)
		{
  		...
  		{
    		if(PeopleBlocking[recieverid][i] == playerid && FoCo_Player[playerid][admin] < 1)
			{
		...
		if((GetPVarInt(recieverid, "PlayerStatus") == 3 && IdleTime[recieverid] > 10) || IdleTime[recieverid] > 30)
		{
			SendClientMessage(playerid, COLOR_NOTICE, "[GUARDIAN]: Looks like the player you are trying to message is AFK/Tabbed.");
		}
    }
    else
    {
...
public OnPlayerUpdate(playerid)
{
	...
	else
	{
	    if(ADuty[playerid] == 0)
	    {
		    SetPlayerHealth(playerid, ServerSideHP[playerid]);
			SetPlayerArmour(playerid, ServerSideAM[playerid]);
		}
	}
	#endif
	return 1;
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    print("ODR_Main");
    new string[512];
    //FOR DIALOG_HIDER_DETECTION, HOOK IS REMOVED FOR OnDialogResponse.
    ...
    //PAS_OnDialogResponse(playerid, dialogid, response, listitem, inputtext); //Crashes in this shit if Its for Security, else fine..
	//FKu_Lvl_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//FKu_Tut_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//Vista_ClickP_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//CAM_SYS_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//CEM_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//DUEL_N_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//CM_List_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//Mod_sql_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//load_arecord_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//ban_handle_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//Report_sys_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//stations_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//ACP_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//gotop_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	
	//Ammunation_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	//BuySkin_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
			//SetStats_OnDialogReponse(playerid, dialogid, response, listitem, inputtext); //Unused
	//SkinMod_OnDialogReponse(playerid, dialogid, response, listitem, inputtext);
			//ClanWar_OnDialogResponse(playerid, dialogid, response, listitem, inputtext); //Unused
    //TurfWar_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
	switch(dialogid)
	{
		case DIALOG_WEATHER:
		{
		...
		case DIALOG_CARMOD4:
		{
			...
			else
			{
				...
				new moneystring[256];
				format(moneystring, sizeof(moneystring), "%s(%d) lost %d$ from Car_Modding.", PlayerName(playerid), playerid, ModCartTotal[playerid]);
...
{
	IdleTime[playerid] = 0;
	WH_OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, fX, fY, fZ);
	#if defined PTS
	RakGuy_OnPlayerWeaponShot_Test(playerid, weaponid, hittype, hitid, fX, fY, fZ);	// This will only run for testserver
	new Float:angle;
	GetPlayerFacingAngle(playerid, angle);
	#endif
	#if !defined PTS
	RakGuy_OnPlayerWeaponShot_Main(playerid, weaponid, hittype, hitid, fX, fY, fZ);	// This will run for main
	#endif
    #if defined SERVERSIDED_HP
	...
	if(hittype == BULLET_HIT_TYPE_PLAYER)
	{
		...
		if(IsPlayerWeaponHacking(playerid, weaponid))
		{
			IgnoredBullet(playerid, hitid, SS_HACKEDWEAPON);
			return 0;
		}
		if(weaponid!=38)
		{
		...
		{
			GameTextForPlayer(playerid, "~r~Either you or his is knife-bugged", 1000, 6);
			GameTextForPlayer(hitid, "~r~Either you or his is knife-bugged", 1000, 6);
		    return 1;
		}
		if(GetTypeOfWeapon(weaponid) == WEAPON_TYPE_RANGED)
		{
		    return 1; // Shotgun damage in fucking GiveDamage
		}
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	if(SS_DeadPlayer[playerid] != true)
	{
	    ...
	    {
			if(weaponid == 4 && amount == 0.0 && KnifeBugged[playerid] == true)
			{
				KnifeBugged[playerid] = false;
				DamageMath(playerid, issuerid, 999.0, weaponid);
			}
		}
		...
		else if(GetTypeOfWeapon(weaponid) == WEAPON_TYPE_FIRE)
		{
		    if(weaponid == 51)
		    {
		        DamageMath(playerid, issuerid, amount, weaponid);
		    }
		    else
		    {
		...
		else if(GetTypeOfWeapon(weaponid) == WEAPON_CAR_RAM)
		{
			if(issuerid != INVALID_PLAYER_ID)
			{
		...
		else if(GetTypeOfWeapon(weaponid) == WEAPON_TYPE_HAND && weaponid == 9)
		{
		    DamageMath(playerid, issuerid, WDamage_Amount[weaponid], weaponid);
		}
	}
	    ...
	    if(damagedid != INVALID_PLAYER_ID)
	    {
			...
			if(IsPlayerInAnyVehicle(damagedid))
			{
				...
				if(GetPlayerDamage(playerid, damagedid, weaponid, Damage_Ttl))
				{
				    if(GetTypeOfWeapon(weaponid) == WEAPON_TYPE_RANGED)
				    {
				        DamageMath(damagedid, playerid, amount, weaponid);
				    }
				    else
				    {
			...
			else if(GetTypeOfWeapon(weaponid) == WEAPON_TYPE_RANGED)
			{
			    new Float:Damage_Ttl;
			    GetPlayerDamage(playerid, damagedid, weaponid, Damage_Ttl);
				if(Damage_Ttl > amount)
				{
					DamageMath(damagedid, playerid, amount, weaponid);
				}
				else
...
//FUNCTIONS BEFORE SSHP WAS MADE
/*
public OnPlayerGiveDamage(playerid, damagedid, Float: amount, weaponid)
{
    return 1;
