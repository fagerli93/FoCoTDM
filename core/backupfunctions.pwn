...
forward TakeMoneyOnKill(playerid, killerid, money);
public TakeMoneyOnKill(playerid, killerid, money)
{
	new string[64];
	if (killerid != INVALID_PLAYER_ID)
	{
		new Float:death_money = ((money / 100) * 0.5);
		new death_money_int = floatround(death_money, floatround_floor);

		if (death_money_int > MAX_DEATH_CASH)
		{
		    TextDrawSetString(MoneyDeathTD[playerid], "~r~-600");
		...
		{
		    TextDrawSetString(MoneyDeathTD[playerid], "~r~-50");
...
forward GiveMoneyOnKill(playerid, killerid);
public GiveMoneyOnKill(playerid, killerid)
{
    new string[64];
	if (playerid != INVALID_PLAYER_ID)
	{
	    if (isVIP(killerid) > 0)
		{
		    ...
		    if(isVIP(killerid) == 1)
			{
				format(string, sizeof(string), "~g~VIP +%d", B_VIP_KILL_BONUS);
				TextDrawSetString(MoneyDeathVIPTD[killerid], string);
				GivePlayerMoney(killerid,B_VIP_KILL_BONUS);
				TextDrawShowForPlayer(killerid, MoneyDeathVIPTD[killerid]);
				defer cashVIPTimer(killerid);
			}
			else if(isVIP(killerid) == 2)
			{
			    format(string, sizeof(string), "~g~VIP +%d",S_VIP_KILL_BONUS);
				TextDrawSetString(MoneyDeathVIPTD[killerid], string);
				GivePlayerMoney(killerid,S_VIP_KILL_BONUS);
				TextDrawShowForPlayer(killerid, MoneyDeathVIPTD[killerid]);
				defer cashVIPTimer(killerid);
			}
			else
			{
			    format(string, sizeof(string), "~g~VIP +%d",G_VIP_KILL_BONUS);
				TextDrawSetString(MoneyDeathVIPTD[killerid], string);
				GivePlayerMoney(killerid,G_VIP_KILL_BONUS);
				TextDrawShowForPlayer(killerid, MoneyDeathVIPTD[killerid]);
				defer cashVIPTimer(killerid);
			}
		}
  		new money = (KILL_CASH + (FoCo_Player[playerid][level]-FoCo_Player[killerid][level])*10);
...
stock GiveGuns(playerid)
{
	...
	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_melee], 1);
	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_handguns], 500);		
	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_shotguns], 250);
	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_submachine], 250);
	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_assault], 250);
	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_rifle], 25);
	new temp_vip =
	(playerid);
	switch(temp_vip)
	{
	    case 1:
	    {	
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_handguns], 250);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_shotguns], 250);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_submachine], 250);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_assault], 250);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_rifle], 25);
	    ...
	    case 2:
	    {		
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_handguns], 375);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_shotguns], 375);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_submachine], 375);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_assault], 375);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_rifle], 37);
		}
		case 3:
		{
 			GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_handguns], 500);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_shotguns], 500);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_submachine], 500);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_assault], 500);
	    	GivePlayerWeapon(playerid, FoCo_Classes[playerid][fc_rifle], 50);
		}
	}
...
{
	new tmpnc;
	tmpnc = FoCo_Players[playerid][nchanges];
	return tmpnc;
}
...
{	
	...
	foreach(Player, i)
	{
		if(viewDamage[playerid][i] == 1 && weaponid != 0 && weaponid != 37)
		{
			if(issuerid == INVALID_PLAYER_ID)
			{
				format(str, sizeof(str), "[Guardian]: %s has taken %f damage, but was not from a player. Must of fell from a height etc", PlayerName(playerid), amount);
